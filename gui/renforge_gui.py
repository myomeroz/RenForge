...
class RenForgeGUI(QMainWindow):
    ...
    def _sync_model_selection(self):
        ...
        logger.debug("[_sync_model_selection] Attempting to sync model selection...")
        current_selection = self.model_combo.currentText()
        if current_selection in ["Loading...", "Error loading models"]:
             logger.debug(f"[_sync_model_selection] Skipping sync due to placeholder: '{current_selection}'")
             return
        default_model = self.settings.get("default_selected_model")

        if not default_model:
            logger.debug("[_sync_model_selection] No default model set in settings. Skipping sync.")

            if self.selected_model != current_selection:
                 logger.debug(f"[_sync_model_selection] Updating self.selected_model to combo box value: '{current_selection}'")
        ...
        if current_selection == default_model:
            logger.debug(f"[_sync_model_selection] Default model '{default_model}' already selected. Sync not needed.")

            if self.selected_model != default_model:
                 logger.debug(f"[_sync_model_selection] Correcting self.selected_model to '{default_model}'")
        ...
        if model_index != -1:
            logger.debug(f"[_sync_model_selection] Found default model '{default_model}' at index {model_index}. Setting selection.")
            ...
            logger.debug(f"[_sync_model_selection] Default model '{default_model}' not found in the current list. Cannot sync. Current selection: '{current_selection}'.")
    ...
    def _load_languages(self):
        ...
        if temp_languages:
             ...
             logger.debug(f"Loaded {len(self.SUPPORTED_LANGUAGES)} languages. Example: {list(self.SUPPORTED_LANGUAGES.items())[:5]}") 
        else:
    ...
    def _update_model_list(self, force_refresh=False):
        ...
        logger.debug(f"[_update_model_list] Called. Force refresh: {force_refresh}")
        ...
        logger.debug(f"[_update_model_list] Target model for selection: '{target_model_to_select}'")
        ...
        if available_models:
            logger.debug(f"[_update_model_list] Populating combo box with {len(available_models)} models.")
            ...
            if target_model_to_select and target_model_to_select != "None":
                ...
                if exact_match_index != -1:
                    ...
                    found_target = True
                    logger.debug(f"[_update_model_list] Found exact match for target model '{target_model_to_select}' at index {selected_index}.")
                else:

                    logger.debug(f"[_update_model_list] Exact match for '{target_model_to_select}' not found. Searching by suffix...")
                    for i in range(1, self.model_combo.count()): 
                        ...
                        if model_name_in_combo.endswith(target_model_to_select):
                            ...
                            found_target = True
                            logger.debug(f"[_update_model_list] Found model '{model_name_in_combo}' ending with target '{target_model_to_select}' at index {selected_index}.")
                            break 

            if not found_target and target_model_to_select != "None":
                 logger.debug(f"[_update_model_list] Target model '{target_model_to_select}' not found. Checking config default '{config.DEFAULT_MODEL_NAME}'...")
                 ...
                 if config_default_index != -1:
                     ...
                     found_target = True
                     logger.debug(f"[_update_model_list] Found exact match for config default model '{config_default_target}'.")
                 else:
                     ...
                     for i in range(1, self.model_combo.count()):
                         ...
                         if model_name_in_combo.endswith(config_default_target):
                             ...
                             target_model_to_select = model_name_in_combo 
                             found_target = True
                             logger.debug(f"[_update_model_list] Found config default model '{model_name_in_combo}' ending with '{config_default_target}'.")
                             break

            if not found_target and self.model_combo.count() > 1: 
                selected_index = 1 
                target_model_to_select = self.model_combo.itemText(selected_index)
                logger.debug(f"[_update_model_list] Neither target nor config default found. Selecting first available: '{target_model_to_select}'.")

            self.model_combo.setCurrentIndex(selected_index)

            current_text_selection = self.model_combo.currentText()
            logger.debug(f"[_update_model_list] Set combo box index to {selected_index} ('{current_text_selection}').")

            self.model_combo.setEnabled(True) 
        else:

            logger.debug("[_update_model_list] Failed to load models or no models available.")
