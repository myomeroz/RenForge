...
if _is_tool_available("unrpa"):
    UNRPA_COMMAND = "unrpa"
    print("INFO: 'unrpa' komutu sistem PATH'inde bulundu.")
else:
    try:
        ...
        if result.returncode == 0 or "usage: unrpa" in result.stderr.lower() or "usage: __main__.py" in result.stderr.lower(): 
            ...
            print("INFO: 'unrpa' modülü 'python -m unrpa' ile kullanılabilir.")
        else:
            print("UYARI: 'unrpa' komutu PATH'te bulunamadı ve 'python -m unrpa' ile erişilemiyor. *.rpa çıkarma kullanılamayacak.")
            print("         'unrpa' kurulumunu (pip install unrpa) ve PATH değişkenini kontrol edin.")
            print(f"         (python -m unrpa --version dönüş kodu: {result.returncode}, stderr: {result.stderr[:100]}...)")
    except FileNotFoundError:
         print(f"UYARI: Python yorumlayıcısı '{sys.executable}' bulunamadı. 'python -m unrpa' kontrol edilemiyor.")
    except subprocess.TimeoutExpired:
         print("UYARI: 'python -m unrpa' kontrolü çok uzun sürdü.")
    except Exception as e:
         print(f"UYARI: 'python -m unrpa' kontrol hatası: {e}")
...
if UNRPYC_SCRIPT_PATH.is_file():
    ...
    if not (UNRPYC_LIB_DIR / "decompiler").is_dir():
         print(f"UYARI: '{UNRPYC_SCRIPT_PATH.name}' betiği bulundu, ancak 'decompiler' klasörü '{UNRPYC_LIB_DIR}' içinde yok.")
         print("         unrpyc'nin TÜM içeriğini 'unrpyc_lib' klasörüne kopyaladığınızdan emin olun.")
    else:
         print(f"INFO: Dekompile betiği '{UNRPYC_SCRIPT_PATH.name}' ve 'decompiler' klasörü '{UNRPYC_LIB_DIR}' içinde bulundu.")
else:
    UNRPYC_AVAILABLE = False
    print(f"UYARI: Dekompile betiği '{UNRPYC_SCRIPT_PATH.name}' '{UNRPYC_LIB_DIR}' içinde bulunamadı.")
    print("         *.rpyc dekompilasyonu kullanılamayacak.")
    print("         unrpyc kütüphanesini (decompiler klasörü dahil) 'utils/unrpyc_lib' klasörüne yerleştirin.")
...
def _extract_single_rpa(rpa_path: Path, project_path: Path, force_extract: bool = False) -> Tuple[bool, str]:
    ...
    if not game_dir.is_dir():
        print(f"UYARI: 'game' klasörü {project_path} içinde bulunamadı. Çıkarma CWD={project_path} ile yapılacak.")
        target_cwd = project_path
    else:
        target_cwd = game_dir

    print(f"INFO: Arşiv çıkarma deneniyor: {rpa_path} (CWD: {target_cwd})")
    ...
    print(f"DEBUG: Komut çalıştırılıyor: {' '.join(map(str, command))}")
    print(f"DEBUG: CWD dizini: {target_cwd}") 
    ...
    try:
        ...
        if result.returncode == 0:
             ...
             if "no such file" in stderr_lower or "no such file" in stdout_lower:
                  ...
                  print(f"UYARI: {error_message}")
                  return False, f"unrpa '{rpa_path.name}' için hata (Kod: 0, ancak çıktıda hata var). Konsolu kontrol edin."
             else:
                  print(f"INFO: '{rpa_path.name}' arşivi {target_cwd} klasöründe başarıyla işlendi.")
                  try:
                      marker_file.touch()
                  except OSError as e:
                      print(f"UYARI: İşaretçi dosyası oluşturulamadı {marker_file}: {e}")
...
def _decompile_single_rpyc(rpyc_path: Path, force_decompile: bool = False) -> Tuple[bool, str]:
    ...
    print(f"INFO: runpy ile dekompilasyon deneniyor: {rpyc_path} (klasör: {rpyc_path.parent})")
    ...
    if unrpyc_lib_path_str not in sys.path:
        ...
        path_added = True
        print(f"DEBUG: sys.path'e geçici olarak eklendi: {unrpyc_lib_path_str}")

    os.chdir(str(rpyc_dir))
    print(f"DEBUG: CWD geçici olarak değiştirildi: {rpyc_dir}")
    ...
    try:
        ...
        print(f"DEBUG: runpy.run_path('{str(UNRPYC_SCRIPT_PATH)}') çalıştırılıyor, argv: {sys.argv}")
        runpy.run_path(str(UNRPYC_SCRIPT_PATH), run_name='__main__')

    except SystemExit as e:
        exit_code = e.code if isinstance(e.code, int) else 1
        print(f"DEBUG: runpy SystemExit ile sonlandı, kod: {exit_code}")
    except Exception as e:
        exit_code = 1
        runpy_error = e
        print(f"HATA: {rpyc_path.name} için runpy çalıştırılırken istisna: {e}")
        import traceback
        traceback.print_exc()
    finally:
        if path_added and unrpyc_lib_path_str in sys.path:
            try:
                 ...
                 print(f"DEBUG: sys.path'ten kaldırıldı: {unrpyc_lib_path_str}")
            except ValueError: 
                 print(f"UYARI: sys.path'ten kaldırılamadı: {unrpyc_lib_path_str}")

        sys.argv = original_argv
        sys.stdout = original_stdout
        sys.stderr = original_stderr
        os.chdir(original_cwd)
        print(f"DEBUG: CWD geri yüklendi: {original_cwd}")

    stdout_content = redirected_stdout.getvalue()
    stderr_content = redirected_stderr.getvalue()
    if stdout_content: print(f"DEBUG: [runpy stdout] {stdout_content.strip()}")
    if stderr_content: print(f"DEBUG: [runpy stderr] {stderr_content.strip()}")

    if exit_code == 0 and runpy_error is None:
        if rpy_path.exists():
             print(f"INFO: {rpyc_path.name} başarıyla dekompile edildi -> {rpy_path.name}")
...
def prepare_project_files(project_path_str: str, settings: dict) -> dict:
    ...
    if not auto_prepare_enabled:
        print("INFO: Otomatik proje hazırlama ayarlarda kapalı.")
        results["preparation_skipped_by_setting"] = True
        return results 

    print("-" * 20)
    print(f"Proje dosya hazırlama başlıyor: {project_path}")

    # Process RPA files
    rpa_files = list(project_path.rglob("*.rpa"))
    if rpa_files:
        if UNRPA_AVAILABLE:
            print(f"{len(rpa_files)} *.rpa dosyası bulundu. İşleniyor...")
            ...
            print(f"UYARI: {len(rpa_files)} *.rpa dosyası bulundu, ancak unrpa kullanılamıyor. Çıkarma atlandı.")
    ...
    if rpyc_files:
        if UNRPYC_AVAILABLE:
            print(f"{len(rpyc_files)} *.rpyc dosyası (__pycache__ dışında) bulundu. İşleniyor...")
            ...
            print(f"UYARI: {len(rpyc_files)} *.rpyc dosyası bulundu, ancak dekompile betiği bulunamadı. Dekompilasyon atlandı.")
            results["rpyc_errors"] = len(rpyc_files) 
            results["rpyc_error_details"].append(f"{UNRPYC_SCRIPT_PATH.name} betiği bulunamadı.")

    print("Proje dosya hazırlama tamamlandı.")
    print(f"  RPA: işlendi={results['rpa_processed']}, atlandı={results['rpa_skipped']}, hata={results['rpa_errors']}")
    print(f"  RPYC: işlendi={results['rpyc_processed']}, atlandı={results['rpyc_skipped']}, hata={results['rpyc_errors']}")
    print("-" * 20)
    return results
...
if __name__ == "__main__":
    ...
    if Path(test_project_dir).is_dir():
        ...
        print("\nHazırlık Sonuçları:")
        print(f"  unrpa kullanılabilir: {prep_results['unrpa_available']}")
        print(f"  unrpyc kullanılabilir: {prep_results['unrpyc_available']}")
        print(f"  RPA İşlendi: {prep_results['rpa_processed']}")
        print(f"  RPA Atlandı: {prep_results['rpa_skipped']}")
        print(f"  RPA Hata: {prep_results['rpa_errors']}")
        if prep_results['rpa_error_details']:
            print("    RPA Hata Detayları:")
            for detail in prep_results['rpa_error_details']: print(f"      - {detail}")
        print(f"  RPYC İşlendi: {prep_results['rpyc_processed']}")
        print(f"  RPYC Atlandı: {prep_results['rpyc_skipped']}")
        print(f"  RPYC Hata: {prep_results['rpyc_errors']}")
        if prep_results['rpyc_error_details']:
            print("    RPYC Hata Detayları:")
            for detail in prep_results['rpyc_error_details']: print(f"      - {detail}")
    else:
